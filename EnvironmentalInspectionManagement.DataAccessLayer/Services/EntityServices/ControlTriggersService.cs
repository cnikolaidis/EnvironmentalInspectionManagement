namespace EnvironmentalInspectionManagement.DataAccessLayer.Services.EntityServices
{
    ...
    public interface IControlTriggersService : IBaseEntityService<ControlTrigger, ControlTriggerCriteria, ControlTriggerDto> { }

    public class ControlTriggersService : BaseEntityService<ControlTrigger, ControlTriggerCriteria, ControlTriggerDto>, IControlTriggersService
    {
        private readonly EiDbContext _dbContext;

        public ControlTriggersService()
        {
            _dbContext = new EiDbContext();
        }

        public override IEnumerable<ControlTrigger> ListByCriteria(ControlTriggerCriteria criteria)
        {
            var query = _dbContext.ControlTriggers.AsQueryable();
        ...
        public override int Create(ControlTriggerDto dto)
        {
            if (dto == null)
            ...
            var existingEntity = ListByCriteria(new ControlTriggerCriteria
            {
                Name = dto.Name
            }).FirstOrDefault();
            ...
            var entityToSave = new ControlTrigger
            {
                Name = dto.Name,
            ...
            var savedEntity = _dbContext.ControlTriggers.Add(entityToSave);
            _dbContext.SaveChanges();

            return savedEntity?.Id ?? 0;
        }

        public override void Update(ControlTriggerDto dto)
        {
            if (dto == null)
                throw new EiEntityException(@"Entity cannot be null");
            if (string.IsNullOrEmpty(dto.Name))
            ...
            var existingEntity = ListByCriteria(new ControlTriggerCriteria { Id = dto.Id }).FirstOrDefault();
        ...
        public override void Delete(int id)
        {
            ...
            var existingEntity = ListByCriteria(new ControlTriggerCriteria { Id = id }).FirstOrDefault();

            if (existingEntity == null)
                throw new EiEntityException($"Error getting entity with Id {id}");

            _dbContext.ControlTriggers.Remove(existingEntity);
            _dbContext.SaveChanges();
        }
    }

    public static class ControlTriggersServiceExtensions
    {
        public static IEnumerable<ControlTriggerDto> GetDtos(this IEnumerable<ControlTrigger> entities)
        {
            var entityDtos = entities
                .Select(x => new ControlTriggerDto()
                {
                    Id = x.Id,
                    Name = x.Name,
